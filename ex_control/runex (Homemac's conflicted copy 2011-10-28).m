...
%%%% ALTERNATE METHOD FOR EYE MOVEMENT - peekdata instead of readEyes
% WHY LAGGY? seems like all the peekdata calls are happening at once
%if params.getEyes    
%    try
%        stop(eyes);
%    end
    
%    eyes = analoginput('mcc');

%    addchannel(eyes,0:1);
        
%    set(eyes,'SamplesPerTrigger',100000);
%    set(eyes,'SampleRate',1000);
...
while 1 
    ...
    elseif c == 's'
        ...
        msgAndWait('ack');
        msg('framerate');
        params.slaveHz = str2double(waitFor());        
        ...
        for j = currentBlock:rpts
            ...
            while ~isempty(ordering)
                ...
                % this is where we might write these codes to a file
