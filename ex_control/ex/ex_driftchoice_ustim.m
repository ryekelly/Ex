function result = ex_driftchoice_ustim(e)
...
% Modified:
%
global params codes behav allCodes trialData;
...
%Query and save the settings of the stimulator:
if ~isfield(behav,'stimSettings')
    username = getenv('username');
    userpath = ['C:\Documents and Settings\' username '\'];
    addpath([userpath 'My Documents\Dropbox\smithlab\S88X\']);
    s88x_initialize;
    behav.stimSettings = s88x_getSettings;
end;
...
trainDuration = str2double(char(behav.stimSettings.S1(9,1:8)))*1000;
stimStatusDisplayRow = 10; %#ok<NASGU>
deepPink = [255 20 147];
...
if isCatch, targAmp=e.temporal; end; %In other words: no target
...
%     trialData{stimStatusDisplayRow} = 'Visual stimulus onset';
%     drawTrialData;

if ~waitForMS((targetOnset.*frameMsec)-trainDuration,e.fixX,e.fixY,params.fixWinRad)
    % failed to keep fixation
    sendCode(codes.BROKE_FIX);
    msgAndWait('all_off');
    sendCode(codes.STIM_OFF);
...
%     trialData{stimStatusDisplayRow} = 'End pre-microstim time';
%     drawTrialData;
yellow = [255 255 0];
fixWinColor = yellow;
if e.ustim,
    ...
    microStim(1)
    fixWinColor = deepPink;
    %         trialData{stimStatusDisplayRow} = 'STIMULATING!';
    %         drawTrialData;
end;

if ~waitForMS(trainDuration,e.fixX,e.fixY,params.fixWinRad,fixWinColor)
    % failed to keep fixation
    sendCode(codes.BROKE_FIX);
    msgAndWait('all_off');
    sendCode(codes.STIM_OFF);
...
%     trialData{stimStatusDisplayRow} = 'End train duration';
%     drawTrialData;

%     trialData{stimStatusDisplayRow} = '';
%     drawTrialData;

hitFlag = false;
winColors = [0,255,0;255,0,0];
%require hold fixation until target onset:
remainingWait = waitAfterTarget.*frameMsec; %(plus a little more to stop false alarms...)
if ~waitForMS(remainingWait,e.fixX,e.fixY,params.fixWinRad)
    % failed to keep fixation
    sendCode(codes.BROKE_FIX);
    msgAndWait('all_off');
    sendCode(codes.STIM_OFF);
...
%     trialData{stimStatusDisplayRow} = 'End wait for target onset';
%     drawTrialData;

%     if e.showHoles %note: this isn't working yet with new catch logic -ACS 29Oct2012
...
if ~hitFlag
    if isCatch
        if ~waitForMS((stimulusDuration-targetOnset).*frameMsec,e.fixX,e.fixY,params.fixWinRad,yellow)
            % failed to keep fixation
            sendCode(codes.BROKE_FIX);
            msgAndWait('all_off');
            sendCode(codes.STIM_OFF);
            ...
            result = codes.BROKE_FIX;
            return;
        else %correct withhold to catch
            msgAndWait('all_off');
            sendCode(codes.WITHHOLD);
            sendCode(codes.STIM_OFF);
            sendCode(codes.FIX_OFF);
        ...
        switch choiceWin
            case 0
                ...
                if waitForFixation(1,e.fixX,e.fixY,params.fixWinRad) %quickly check the fixation window
                    sendCode(codes.NO_CHOICE);
                    msgAndWait('all_off');
                    behav.score(end+1) = 0; %count a no-choice like a wrong-choice (makes the target easier)
                    behav.trialNum(end+1) = length(allCodes);
                    behav.RT(end+1) = nan;
...
trialData{7} = sprintf('Hits: %d, Incorrects: %d (%.1f%%), Other: %d',sum(behav.score==1&~isnan(behav.score)),sum(behav.score==0&~isnan(behav.score)),nanmean(behav.score)*100,sum(isnan(behav.score)));
trialData{8} = sprintf('Target amplitudes: %.2f, %.2f',behav.targAmp);
